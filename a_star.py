...
def calc_hueristic_cost(node, containers_to_unload, containers_to_load_count, col, row_idx_of_top_container, last_operation, cur_operation, move_coord, crane_cords):
    h = 0
    
    # heavily penalize going for 2 loads in a row while theres a unload
    if containers_to_unload and last_operation == 'Load' and cur_operation == 'Load':
        h += 50
    
    # penalize going for 2 unloads in a row while theres a load
    if containers_to_load_count and last_operation == 'Unload' and cur_operation == 'Unload':
        h += 30
    
    # incentivise going for load then unload
    if containers_to_unload and (last_operation == 'Load' and cur_operation == 'Unload') or (last_operation == 'Unload' and cur_operation == 'Load'):
        h -= 40
    ...
    if cur_operation == 'Move':
        ...
        h += manhattan_distance(move_coord, crane_cords)
        
    h -= len(containers_to_unload) * 5
    h -= containers_to_load_count * 5    # tried *3 and 5 is faster
...
def manhattan_distance(point1, point2):
    distance = 0
    for i in range(len(point1)): # (valid_row,col),(-1,0)
        ...
        for otherCol in range(columns):
            ...
            if otherRow != None and myCol != otherCol:
                ...
                if tuple(map(tuple, new_state)) not in explored_states:
                    ...
                    child.set_cost_h(calc_hueristic_cost(child,containers_to_unload,len(containers_to_load), col,row_idx_of_top_container, 'Load', 'Move', (otherRow, otherCol), (crane_cords[0],crane_cords[1])))
                    ...
                    child.set_operation_info((location_from,location_to, manhattan_distance((myRow,myCol),(otherRow,otherCol)) + manhattan_distance((crane_cords[0],crane_cords[1]),(myRow,myCol))))
    ...
    if containers_to_load: #check if load list is empty
        ...
        for col in range(columns):
            ...
            if valid_row is not None:                
                distance = manhattan_distance((valid_row, col), (-1, 0))
            ...
            if valid_row is not None:
                ...
                for container in containers_to_load:
                    ...
                    if tuple(map(tuple,new_state)) not in explored_states:
                        ...
                        child.set_cost_h(calc_hueristic_cost(child,containers_to_unload,len(temp_list_load), col, row_idx_of_top_container, last_operation, 'Load', (valid_row,col), (crane_cords[0],crane_cords[1])) + node.get_cost_h())
                        ...
                        child.set_operation_info((location_from,location_to,manhattan_distance((valid_row,col),(-1,0)) + manhattan_distance((crane_cords[0],crane_cords[1]),(-1,0)) + 4))
                        children.append(child)
                    ...
                    if container[1] == node.state[row][col][1]:
                        ...
                        if tuple(map(tuple,new_state)) not in explored_states:
                            ...
                            if child.crane_location == (-1,0):
                                last_operation = 'Unload'
                            child.set_cost_h(calc_hueristic_cost(child,temp_list_unload,len(containers_to_load), col, row_idx_of_top_container, last_operation, 'Unload', (-1, -1), (crane_cords[0],crane_cords[1])) + node.get_cost_h())
                            ...
                            child.set_operation_info((location_from,location_to, manhattan_distance((row,col),(-1,0)) + manhattan_distance((crane_cords[0],crane_cords[1]),(row,col)) + 4))
                            children.append(child)
